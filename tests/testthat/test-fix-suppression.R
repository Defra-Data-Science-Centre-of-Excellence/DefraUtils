...
test_df_zeros_simple <- tibble::tribble(
  ~group_a, ~group_b, ~group_c, ~group_d, ~value, ~sample_size,
  "a1",     "b1",     "c1",     "d1",     16.2,     0,
  "a1",     "b2",     "c2",     "d2",     5.6,      2,
  "a1",     "b3",     "c2",     "d1",     2.3,     10,
...
test_df_zeros_complex <- tibble::tribble(
  ~group_a, ~group_b, ~group_c, ~group_d, ~value, ~sample_size,
  "a1",     "b1",     "c2",     "d",      16.2,   2,
  "a1",     "b2",     "c2",     "d",      5.6,    3,
  "a1",     "b3",     "c2",     "d",      2.3,    0,
...
test_that("simple (one-level) suppression works as expected", {
  ...
  # confirming that the value column is not changed
  
  expect_equal(fix_suppression(df = test_df, groups = list("group_a"))$value,
               test_df$value)
  expect_equal(fix_suppression(df = test_df, groups = list("group_b"))$value,
               test_df$value)
...
test_that("two-level suppression works as expected", {
  ...
  expect_equal(fix_suppression(df = test_df, groups = list("group1" <- c("group_a"),
                                                           "group2" <- c("group_b")
                                                           ))$sample_size,
               c(2, 3, 1, 12, 1, 1, 1, 152))
  
  expect_no_message(fix_suppression(df = test_df, groups = list("group1" <- c("group_a"),
                                                                "group2" <- c("group_b")
                                                                )),
  message = "Suppression loop 2")
...
test_that("one-level suppression with 0's in data set works as expected", {
  
  expect_equal(fix_suppression(df = test_df_zeros_simple, groups = list("group1" <- c("group_b")
                                                                        )
                               )$sample_size,
               c(1, 1, 10, 12, 1, 1, 8, 152))
  
  expect_equal(fix_suppression(df = test_df_zeros_simple, groups = list("group1" <- c("group_a")
                                                                        )
                               )$sample_size,
               c(0, 1, 1, 12, 1, 0, 1, 152))
  
  expect_no_message(fix_suppression(df = test_df_zeros_simple, groups = list("group1" <- c("group_a")
                                                                             )),
                    message = "Suppression loop 2")
  
  expect_no_message(fix_suppression(df = test_df, groups = list("group2" <- c("group_b")
                                                                )),
                    message = "Circular suppression complete")
  })


test_that("one-level suppression with 0's in data set works as expected, and an orphan", {
  
  expect_equal(fix_suppression(df = test_df_zeros_simple, groups = list("group1" <- c("group_c")
                                                                        )
                               )$sample_size,
               c(0, 1, 1, 12, 1, 0, 8, 152))
  
  expect_equal(fix_suppression(df = test_df_zeros_simple, groups = list("group1" <- c("group_d")
                                                                        )
                               )$sample_size,
               c(0, 2, 1, 12, 1, 0, 8, 152))
  })

test_that("two-level suppression with 0's in data set works as expected", {
  
  expect_equal(fix_suppression(df = test_df_zeros_simple, groups = list("group1" <- c("group_a"),
                                                                        "group2" <- c("group_b")
                                                                        )
                               )$sample_size,
               c(1, 1, 1, 12, 1, 1, 1, 152))
...
test_that("a breakdown that requires circular suppression works as expected", {
  ...
  expect_message(suppressWarnings(
    fix_suppression(df = test_df_zeros_complex,
                    groups = list(c("group_d", "group_c", "group_b"),
                                  c("group_d", "group_c", "group_a")))),
    "The input groups or data require circular suppression")
  ...
  expect_warning(fix_suppression(df = test_df_zeros_complex,
                                 groups = list(c("group_d", "group_c", "group_b"),
                                               c("group_d", "group_c", "group_a"))),
                 "Circular suppression is required, but save_excel_file is FALSE")
  ...
  expect_equal(suppressWarnings(
    fix_suppression(df = test_df_zeros_complex,
                    groups = list(c("group_d", "group_c", "group_b"),
                                  c("group_d", "group_c", "group_a"))))$sample_size,
    c(2, 3, 1, 12, 1, 1, 1, 152, 1, 1, 5, 1, 1, 1, 8, 1, 1, 0, 1, 9, 1, 9, 1, 95))
